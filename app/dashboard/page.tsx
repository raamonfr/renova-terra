...
export default function DashboardPage() {
  ...
  // --- ESTADOS (Declarados explicitamente para evitar o erro 'not defined') ---
  const [user, setUser] = useState<UserData | null>(null);
  const [myCommittees, setMyCommittees] = useState<Committee[]>([]);
  const [allCommittees, setAllCommittees] = useState<Committee[]>([]);
  const [memberCommitteeIds, setMemberCommitteeIds] = useState<Set<string>>(
    new Set()
  );
  ...
  const loadData = async () => {
    try {
      ...
      const { data: createdCommittees } = await supabase
        .from("committees")
        .select("*")
      ...
      const { data: allCommitteesData } = await supabase
        .from("committees")
        .select("*");
  ...
  const handleCreateCommittee = async () => {
    ...
    try {
        ...
        .insert([
          {
            ...
            gallery_images:
              newCommittee.galleryImages.length > 0
                ? newCommittee.galleryImages
                : null,
          },
        ])
        .select()
      ...
      await supabase
        .from("committee_members")
        .insert([{ committee_id: data.id, user_id: user.id }]);
      ...
      setFeedbackMessage({
        type: "error",
        message: "Erro ao criar comitê. Tente novamente.",
      });
      setTimeout(() => setFeedbackMessage(null), 3000);
    }
  };
  ...
  const handleJoinCommittee = async () => {
    ...
    try {
      ...
      setFeedbackMessage({
        type: "success",
        message: `Você agora é membro do ${selectedCommittee.name}!`,
      });
      setTimeout(() => setFeedbackMessage(null), 3000);
      ...
      setFeedbackMessage({
        type: "error",
        message: "Erro ao entrar no comitê. Tente novamente.",
      });
      setTimeout(() => setFeedbackMessage(null), 3000);
    }
  };
  ...
  return (
    // ADICIONADO: overflow-x-hidden para evitar o corte lateral no mobile
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <header className="border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50">
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-background border-t border-border absolute w-full left-0 top-full shadow-lg z-50 p-4 flex flex-col gap-4">
            <Link
              href="/dashboard"
              className="text-sm font-medium text-primary py-2 border-b border-border/50"
      ...
      <div className="container mx-auto px-4 py-8">
        {/* Greeting */}
        <div className="mb-8">
          {/* ADICIONADO break-words para evitar corte de nomes longos */}
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2 break-words">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Stats Cards - MODIFICADO para grid de 1 coluna em mobile e 3 em desktop */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Card className="border-border bg-gradient-to-br from-accent/10 to-accent/5 overflow-hidden">
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between mb-4">
              ...
              <Card className="border-border bg-gradient-to-br from-secondary/30 to-secondary/10 overflow-hidden">
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="h-12 w-12 rounded-full bg-primary/20 flex items-center justify-center">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="space-y-1">
                                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                    ...
                                    <span>
                                      {formatDate(event.event_date)} às{" "}
                ...
                <div className="space-y-3">
                  {myCommittees.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground text-center py-4">
                      Você ainda não criou nenhum comitê
                    </p>
                  )}
                </div>
              </CardContent>
          ...
          {createStep === 1 && (
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  {newCommittee.name.length}/50 caracteres
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  {newCommittee.description.length}/300 caracteres
                </p>
              </div>
            </div>
          )}
      ...
      <Dialog open={showEventModal} onOpenChange={setShowEventModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você deseja confirmar sua presença no evento "
              {selectedEvent?.title}"?
            </DialogDescription>
          </DialogHeader>
          {selectedEvent && (
